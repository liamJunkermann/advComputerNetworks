...
\begin{document}
    ...
    \label{sec:Design}
    This webproxy has 3 main parts: the main server (run from \coderef{main.go}), the cache (built in \coderef{cache.go}) and the web dashboard which manages blocking of urls (run from \coderef{webDashboard.go} and \coderef{dynamicBlock.go})
    \newpage
